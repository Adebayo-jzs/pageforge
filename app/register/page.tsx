  ...
  return (
    <main className="min-h-screen bg-landing-bg flex flex-col lg:flex-row font-dmsans">
      ...
      <div className="hidden lg:flex flex-1 relative flex-col justify-between p-16 border-r border-landing-border bg-white/40 overflow-hidden">
        ...
        <div className="relative z-10 max-w-md">
          ...
          <h2 className="font-instrument text-5xl text-landing-ink leading-tight tracking-tight mb-8">
            ...
            <em className="italic text-landing-accent not-italic">vision</em> today.
          </h2>
          <p className="text-landing-ink-muted text-base font-[350] leading-relaxed">
            Create an account to save your projects, download source code, and refactor with AI at any time.
          </p>
        </div>
        ...
        <div className="relative z-10 flex gap-12">
          {[
            { n: "Free", label: "starter plan" },
            { n: "∞", label: "projects" },
            { n: "Clean", label: "HTML/Tailwind" },
          ].map((s) => (
            <div key={s.label}>
      ...
      <div className="w-full lg:w-[480px] flex flex-col justify-center p-8 md:p-16 bg-landing-bg">
        ...
        {/* Form */}
        <form onSubmit={handleRegister} className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300">
          <div>
            <label className="block text-[0.7rem] font-bold text-landing-ink tracking-widest uppercase mb-2 ml-1">Full Name</label>
            <input
            ...
            <div className="relative">
              ...
              {password.length > 0 && (
                <div className="absolute right-4 top-1/2 -translate-y-1/2 flex gap-1">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className={`w-3.5 h-1 rounded-full transition-all ${password.length >= i * 3 ? (i <= 1 ? "bg-red-400" : i <= 2 ? "bg-orange-400" : i <= 3 ? "bg-yellow-400" : "bg-green-400") : "bg-landing-border"}`} />
                  ))}
                </div>
              )}
            </div>
        ...
        <div className="mt-12 flex flex-col items-center gap-4 animate-in fade-in duration-700 delay-400">
          <p className="text-sm text-landing-ink-muted">
            ...
            <Link href="/login" className="text-landing-accent font-bold hover:underline">
              Sign in
            </Link>
          </p>
